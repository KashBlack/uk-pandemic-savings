...
def main():
    ...
    with st.expander("ℹ️ About the Data"):
        ...
        **Coverage**:
        ...
        - Pre-pandemic baseline (2016-2019): £{df_yearly.filter(pl.col('survey_year').is_between(2016, 2019))['avg_savings'].mean():.0f}/HH/year
        
        **Methodology**:
        - Excess savings = Actual savings - Pre-pandemic baseline (2016-2019)
        - Uses savings rate proxy (8% pre-pandemic, 18% during, 10% post)
        - Extrapolated to UK population (28M households)
    ...
    baseline_avg = df_yearly.filter(
        pl.col('survey_year').is_between(2016, 2019)
    )['avg_savings'].mean()
    
    # Pandemic period (2020-2021)
    pandemic_avg_savings = df_yearly.filter(
        pl.col('survey_year').is_between(2020, 2021)
    )['avg_savings'].mean()
    pandemic_excess = df_yearly.filter(
        pl.col('survey_year').is_between(2020, 2021)
    )['avg_excess_savings'].mean()
    
    # Post-pandemic (2022+)
    post_avg_savings = df_yearly.filter(
        pl.col('survey_year') >= 2022
    )['avg_savings'].mean()
    post_excess = df_yearly.filter(
        pl.col('survey_year') >= 2022
    )['avg_excess_savings'].mean()
    ...
    col1.metric(
        ...
        "vs BoE estimate: £200bn",
        delta_color="off"
    )
    col2.metric(
        "Pre-Pandemic Baseline",
        f"£{baseline_avg:.0f}/HH/year",
        "2016-2019 average"
    )
    col3.metric(
        "Pandemic Peak (2020-21)",
        f"£{pandemic_avg_savings:.0f}/HH/year",
        f"+£{(pandemic_avg_savings - baseline_avg):.0f} vs baseline"
    )
    col4.metric(
        "Current (2022+)",
        f"£{post_avg_savings:.0f}/HH/year",
        f"+£{(post_avg_savings - baseline_avg):.0f} vs baseline"
    )
    ...
    with tab1:
        ...
        with col2:
            ...
            **Why the difference?**
            - NMG survey has limited coverage
            - Some wealth held outside surveyed accounts
            - BoE uses multiple data sources
            """)
